...
# ==========================================
# 1. 数据集定义 (精准提取11帧共33通道 + 16种裁剪(4固定+12随机))
# ==========================================
    ...
    def __init__(self, svi_dir, hsi_dir, sample_indices, is_train=False):
        ...
        for idx in sample_indices:
            ...
            try:
                ...
                # --- 提取指定的 11 帧 (共33通道) ---
                target_frames = [1, 30, 44, 55, 65, 75, 84, 92, 101, 132, 146]
                frame_indices = []
                for f in target_frames:
                    # Python 是 0 索引，真实位置减 1
                    idx_0_based = f - 1
                    # 分别提取该帧的 R, G, B 通道
                    frame_indices.extend([idx_0_based, idx_0_based + 183, idx_0_based + 366])
...
class MobileHSINet(nn.Module):
    def __init__(self, in_ch=33, feat_ch=64, num_blocks=4, out_ch=284):
...
def train_and_evaluate():
    ...
    save_pred_dir = r"E:\songweiran\Imaging\Data20260401\Video\predicted_colour"
    results_dir = r"E:\songweiran\Imaging\Data20260401\Results"

    os.makedirs(save_pred_dir, exist_ok=True)
    os.makedirs(results_dir, exist_ok=True)

    # 结果保存路径设定 (统一命名为 colours2HSI 相关)
    model_save_path = os.path.join(results_dir, "best_model_colours2HSI.pth")
    csv_save_path = os.path.join(results_dir, "loss_log_colours2HSI.csv")
    ...
    # 模型初始化 (注意此处的 in_ch 为 33)
    model = MobileHSINet(in_ch=33, feat_ch=64, num_blocks=4, out_ch=284).to(device)
    criterion = nn.L1Loss()
    ...
    plt.plot(range(1, epochs + 1), history_train_loss, label='Train MAE (colours2HSI)', color='blue')
    plt.plot(range(1, epochs + 1), history_val_loss, label='Validation MAE (colours2HSI)', color='orange')
    plt.xlabel('Epoch')
    plt.ylabel('Mean Absolute Error (MAE)')
    plt.title('Training and Validation Loss Curve (33-Channel)')
    plt.legend()
    plt.grid(True)
    plt.show()
    ...
    print("\n" + "=" * 50)
    print(f"[Results] 基于 11帧多光谱 (33通道) 的 HSI 重建测试集平均性能指标：")
...
def visualize_results(single_pred, single_gt, sample_id, results_dir):
    ...
    plt.suptitle(f"Reconstruction Performance - Sample ID: {sample_id} (colours2HSI Baseline)", fontsize=16)
    ...
    plt.plot(wavelengths, curve_pred, label='Predicted (colours2HSI)', color='red', linestyle='--')
    ...
    plt.tight_layout()
    # 保存可视化图片，命名带有 colours2HSI 标识
    save_fig_path = os.path.join(results_dir, f"colours2HSI_Result_Sample_{sample_id}.png")
