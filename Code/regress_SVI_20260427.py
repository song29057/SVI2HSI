...
def load_data_and_generate_folds(data_dir):
    ...
    if X_data.shape[1] != 552:
        print(f"[Warning] X_SVI 特征维度为 {X_data.shape[1]}，期望为 552。")
...
def train_and_evaluate():
    ...
    feature_save_path = os.path.join(feature_dir, "SVI_baseline_features_CV.csv")
    ...
    print("\n[Data Split Info] 5折交叉验证样品分配:")

    for f in range(1, 6):
        print(f"Fold {f} 测试样品: {folds[f]}")
    ...
    for fold in range(1, 6):
        ...
        train_dataset_full = SVIDataset(X_train_full, Y_train_full)
        ...
        test_dataset = SVIDataset(X_test, Y_test)
    ...
    print(f"[Output] SVI基线预测结果及特征已保存至: {pred_save_path} / {feature_save_path}")
    ...
    print("\n" + "=" * 60)
    print("[Results Summary] 5折交叉验证 SVI Baseline 模型 OOF 指标:")
