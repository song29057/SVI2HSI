...
def train_and_evaluate():
    ...
    save_pred_dir = r"E:\songweiran\Imaging\Data20260326\Video\predicted_feature"
    ...
    # 数据集划分 (100个样本: 70训练, 15验证, 15测试)
    train_indices = list(range(1, 71))  # 1 到 70
    val_indices = list(range(71, 86))  # 71 到 85
    test_indices = list(range(86, 101))  # 86 到 100
    ...
    for epoch in range(epochs):
        ...
        for svi, hsi in train_loader:
            ...
            hsi = torch.clamp(hsi / 3956.0, 0.0, 1.0)
    ...
    with torch.no_grad():
        for svi, hsi in test_loader:
            ...
            for b in range(svi.size(0)):
                ...
                # 保存预测结果 (.mat 格式)
                pred_save_raw = single_pred.cpu().numpy().transpose(1, 2, 0) * 3956.0
