...
class HSI_Baseline_Net(nn.Module):
    def __init__(self, input_dim=616, hidden_dim=32):
        super(HSI_Baseline_Net, self).__init__()
...
def train_and_evaluate():
    ...
    for fold in range(1, 6):
        ...
        train_loader = DataLoader(HSIDataset(X_train, Y_train), batch_size=64, shuffle=True)
        # 在 K-Fold 中，用每一折的独立测试集作为验证集来监控泛化能力
        val_loader = DataLoader(HSIDataset(X_test, Y_test), batch_size=64, shuffle=False)
    ...
    pred_df = pd.DataFrame(oof_details, columns=["Fold_ID", "True_Label", "Predicted_Label_HSI"])
    ...
    print("\n" + "=" * 60)
    print(" [Results Summary] 5折交叉验证 HSI 微型残差基线模型 评估结果:")
    print("-" * 60)
    ...
    plt.title('Average Training and Validation Loss Curve (HSI CV)')
    plt.legend()
    plt.grid(True)
    ...
    plt.title('Predicted vs. True Values with Variance (HSI 5-Fold CV)')
    plt.legend()
    plt.grid(True)
