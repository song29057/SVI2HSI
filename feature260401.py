...
import scipy.io as sio  # 保留用于保存最终的预测结果 (.mat)
import numpy as np
...
class SVI2HSIDataset(Dataset):
    def __init__(self, svi_dir, hsi_dir, sample_indices, is_train=False):
        ...
        for idx in sample_indices:
            ...
            try:
                ...
                hsi_data = np.load(hsi_path).astype(np.float32)
    ...
    def __len__(self):
        # 训练集: 每个文件切分为 16 块 (4固定 + 12随机)，数据量扩大 16 倍
        if self.is_train:
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train:
            ...
            crop_idx = idx % 16  # 0~3 为四个角，4~15 为随机位置
        else:
...
class MobileResidualBlock(nn.Module):
    def __init__(self, channels):
        ...
        # 修正：去除了 BN 层，因此必须将 bias 改为 True
...
class SpectralFeatureExtractor(nn.Module):
    """
    轻量级特征映射网络：结合独立分组 ECA 通道注意力和 1x1 卷积
    完美适配 [R1..R183, G1..G183, B1..B183] 数据结构
    """

    def __init__(self, in_ch=549, compressed_ch=32, feat_ch=64, frames=183):
        ...
        # 2. 1x1 卷积: 线性组合 549 维并严格降维到 32 维
    ...
    def forward(self, x):
        ...
        # 2. 变换维度: [b, 549, 1, 1] -> [b, 3, 183]
        y = y.view(b, 3, self.frames)

        # 3. 分组 1D 卷积
        y = self.eca_conv(y)

        # 4. 恢复原有维度: [b, 3, 183] -> [b, 549, 1, 1]
        y = y.view(b, c, 1, 1)
        weight = self.sigmoid(y)

        # 5. 特征加权 -> 降维至32 -> 空间特征提取至64
        x = x * weight
...
def train_and_evaluate():
    # 数据集和输出路径配置
    svi_dir = r"E:\songweiran\Imaging\Data20260401\Video\npy"
    hsi_dir = r"E:\songweiran\Imaging\Data20260401\HSI\npy"

    # 修改1: 预测文件保存路径改为 predicted_feature
    save_pred_dir = r"E:\songweiran\Imaging\Data20260401\Video\predicted_feature"
    results_dir = r"E:\songweiran\Imaging\Data20260401\Results"

    os.makedirs(save_pred_dir, exist_ok=True)
    os.makedirs(results_dir, exist_ok=True)

    # 修改2: 结果保存路径设定统一命名为 features2HSI
    model_save_path = os.path.join(results_dir, "best_model_features2HSI.pth")
    csv_save_path = os.path.join(results_dir, "loss_log_features2HSI.csv")

    # 数据集划分
    train_indices = list(range(1, 31)) + list(range(41, 71))
    val_indices = list(range(31, 36)) + list(range(71, 76))
    test_indices = list(range(36, 41)) + list(range(76, 81))
    ...
    # 模型初始化: in_ch=549, compressed_ch=32 对齐基线 33维
    ...
    for epoch in range(epochs):
        ...
        for svi, hsi in train_loader:
            svi, hsi = svi.to(device), hsi.to(device)

            # --- 修改：在 GPU 端进行归一化和裁剪，降低 CPU 负载 ---
            svi = torch.clamp(svi / 255.0, 0.0, 1.0)
            hsi = torch.clamp(hsi / 4095.0, 0.0, 1.0)
    ...
    plt.plot(range(1, epochs + 1), history_train_loss, label='Train MAE (features2HSI)', color='blue')
    plt.plot(range(1, epochs + 1), history_val_loss, label='Validation MAE (features2HSI)', color='orange')
    plt.xlabel('Epoch')
    plt.ylabel('Mean Absolute Error (MAE)')
    plt.title('Training and Validation Loss Curve (549 to 32-Channel Compression)')
    plt.legend()
    plt.grid(True)
    plt.show()
    ...
    with torch.no_grad():
        for svi, hsi in test_loader:
            ...
            for b in range(svi.size(0)):
                ...
                # 保存预测结果 (.mat 格式)
                pred_save_raw = single_pred.cpu().numpy().transpose(1, 2, 0) * 4095.0
    ...
    print("\n" + "=" * 50)
    print(f"[Results] 基于 549 维 ECA 并压缩至 32 维的 HSI 重建测试集平均性能指标：")
...
def visualize_results(single_pred, single_gt, sample_id, results_dir):
    ...
    plt.suptitle(f"Reconstruction Performance - Sample ID: {sample_id} (features2HSI 32D Baseline)", fontsize=16)
    ...
    plt.plot(wavelengths, curve_pred, label='Predicted (features2HSI)', color='red', linestyle='--')
    ...
    plt.tight_layout()
    # 保存可视化图片，命名带有 features2HSI 标识
    save_fig_path = os.path.join(results_dir, f"features2HSI_Result_Sample_{sample_id}.png")
